...
class Limb(Base.StartClass, Layout.LayoutClass):
    ...
    def createGuide(self, *args):
        ...
        cmds.addAttr(self.moduleGrp, longName="type", attributeType='enum',
                     enumName=self.langDic[self.langName]['m028_arm'] + ':' + self.langDic[self.langName]['m030_leg'])
        ...
        cmds.addAttr(self.moduleGrp, longName="style", attributeType='enum',
        ...
        cmds.parent(self.cvBeforeLoc, self.cvMainLoc, self.cornerGrp, self.cvExtremLoc, self.cvUpVectorLoc,
                    self.moduleGrp)
        ...
        cmds.parentConstraint(self.cvMainLoc, self.jGuideMain, maintainOffset=False,
                              name=self.jGuideMain + "_ParentConstraint")
        cmds.parentConstraint(self.cvCornerLoc, self.jGuideCorner, maintainOffset=False,
                              name=self.jGuideCorner + "_ParentConstraint")
        cmds.parentConstraint(self.cvExtremLoc, self.jGuideExtrem, maintainOffset=False,
                              name=self.jGuideExtrem + "_ParentConstraint")

        # align cornerLocs:
        cmds.aimConstraint(self.cvExtremLoc, self.cornerGrp, aimVector=(0.0, 0.0, 1.0), upVector=(0.0, -1.0, 0.0),
                           worldUpType="object", worldUpObject=self.cvUpVectorLoc,
                           name=self.cornerGrp + "_AimConstraint")
        ...
        cornerPointConst = \
        cmds.pointConstraint(self.cvMainLoc, self.cvExtremLoc, self.cornerPointGrp, maintainOffset=False,
                             name=self.cornerPointGrp + "_PointConstraint")[0]
        ...
        cmds.pointConstraint(self.cvExtremLoc, self.cvUpVectorGrp, maintainOffset=True,
                             name=self.cvUpVectorGrp + "_PointConstraint")
    ...
    def reCreateEditSelectedModuleLayout(self, bSelect=False, *args):
        ...
        self.typeLayout = cmds.rowLayout(numberOfColumns=4, columnWidth4=(100, 50, 77, 70),
                                         columnAlign=[(1, 'right'), (2, 'left'), (3, 'right')], adjustableColumn=4,
                                         columnAttach=[(1, 'both', 2), (2, 'left', 2), (3, 'left', 2), (3, 'both', 10)],
        ...
        self.reOrientBT = cmds.button(label=self.langDic[self.langName]['m022_reOrient'],
                                      annotation=self.langDic[self.langName]['m023_reOrientDesc'],
                                      command=self.reOrientGuide, parent=self.typeLayout)
        ...
        styleMenuItemList = [self.langDic[self.langName]['m042_default'], self.langDic[self.langName]['m026_biped'],
                             self.langDic[self.langName]['m037_quadruped'],
                             self.langDic[self.langName]['m043_quadSpring']]
        ...
        # bend layout:
        self.bendLayout = cmds.rowLayout(numberOfColumns=4, columnWidth4=(100, 20, 50, 20),
        ...
        self.bendChkbox = cmds.checkBox(value=self.getHasBend(), label=' ', ofc=self.setBendFalse, onc=self.setBendTrue,
                                        parent=self.bendLayout)
        self.bendNumJointsMenu = cmds.optionMenu("bendNumJointsMenu", label='Ribbon Joints',
                                                 changeCommand=self.changeNumBend, enable=self.getHasBend(),
                                                 parent=self.bendLayout)
    ...
    def reOrientGuide(self, *args):
        ...
        # re-orient rotations:
        tempToDelBefore = cmds.aimConstraint(self.cvMainLoc, self.cvBeforeLoc, aimVector=(0.0, 0.0, 1.0),
                                             upVector=(1.0, 0.0, 0.0))
        tempToDelMain = cmds.aimConstraint(self.cvCornerLoc, self.cvMainLoc, aimVector=(0.0, 0.0, 1.0),
                                           upVector=(1.0, 0.0, 0.0))
    ...
    def rigModule(self, *args):
        ...
        if cmds.objExists(self.moduleGrp):
            ...
            for s, side in enumerate(sideList):
                ...
                if self.limbStyle == self.langDic[self.langName]['m037_quadruped'] or self.limbStyle == \
                        self.langDic[self.langName]['m043_quadSpring']:
                    self.cvLocList = [self.cvBeforeLoc, self.cvMainLoc, self.cvCornerLoc, self.cvCornerBLoc,
                                      self.cvExtremLoc]
                ...
                for n, jName in enumerate(self.jNameList):
                    if n == 0:
                        fkCtrl = cmds.circle(name=side + self.userGuideName + "_" + jName + "_Ctrl", ch=False, o=True,
                                             nr=(0, 0, 1), d=1, s=8, radius=self.ctrlRadius)[0]
                    else:
                        fkCtrl = \
                        cmds.circle(name=side + self.userGuideName + "_" + jName + "_Fk_Ctrl", ch=False, o=True,
                                    nr=(0, 0, 1), d=1, s=8, radius=self.ctrlRadius)[0]
                    ...
                    utils.originedFrom(objName=origGrp,
                                       attrString=self.cvLocList[n][self.cvLocList[n].find("__") + 1:].replace(":",
                                                                                                               "_"))
                    cmds.parentConstraint(self.skinJointList[n], origGrp, maintainOffset=False,
                                          name=origGrp + "_ParentConstraint")
                    if n > 1:
                ...
                for n in range(len(self.jNameList)):
                    ...
                    cmds.makeIdentity(self.skinJointList[n], self.ikJointList[n], self.ikNSJointList[n],
                                      self.fkJointList[n], apply=True, rotate=True)
                    # fk control leads fk joint:
                    if n == 0:
                        cmds.parentConstraint(self.fkCtrlList[n], self.fkJointList[n], maintainOffset=True,
                                              name=side + self.userGuideName + "_" + self.jNameList[
                                                  n] + "_ParentConstraint")
                    else:
                        cmds.parentConstraint(self.fkCtrlList[n], self.fkJointList[n], maintainOffset=True,
                                              name=side + self.userGuideName + "_" + self.jNameList[
                                                  n] + "_Fk_ParentConstraint")
                ...
                self.worldRef = \
                cmds.circle(name=side + self.userGuideName + "_WorldRef", ch=False, o=True, nr=(0, 1, 0), d=3, s=8,
                            radius=self.ctrlRadius)[0]
                cmds.addAttr(self.worldRef, longName=side + self.limbType + str(dpAR_count) + '_IkFkBlend',
                             attributeType='float', minValue=0, maxValue=1, defaultValue=0, keyable=True)
                if not cmds.objExists(self.worldRef + '.globalStretch'):
                    cmds.addAttr(self.worldRef, longName='globalStretch', attributeType='float', minValue=0, maxValue=1,
                ...
                cmds.pointConstraint(self.shoulderNullGrp, self.zeroFkCtrlList[1], maintainOffset=True,
                                     name=self.zeroFkCtrlList[1] + "_PointConstraint")
                fkIsolateParentConst = \
                cmds.parentConstraint(self.shoulderNullGrp, self.worldRef, self.zeroFkCtrlList[1],
                                      skipTranslate=["x", "y", "z"], maintainOffset=True,
                                      name=self.zeroFkCtrlList[1] + "_ParentConstraint")[0]
                cmds.addAttr(self.fkCtrlList[1], longName=self.langDic[self.langName]['c_Follow'],
                             attributeType='float', minValue=0, maxValue=1, defaultValue=0, keyable=True)
                cmds.connectAttr(self.fkCtrlList[1] + '.' + self.langDic[self.langName]['c_Follow'],
                                 fkIsolateParentConst + "." + self.shoulderNullGrp + "W0", force=True)
                self.fkIsolateRevNode = cmds.createNode('reverse', name=side + self.userGuideName + "_FkIsolate_Rev")
                cmds.connectAttr(self.fkCtrlList[1] + '.' + self.langDic[self.langName]['c_Follow'],
                                 self.fkIsolateRevNode + ".inputX", force=True)
                cmds.connectAttr(self.fkIsolateRevNode + '.outputX', fkIsolateParentConst + "." + self.worldRef + "W1",
                                 force=True)
                ...
                for n in range(len(self.jNameList)):
                    if n > 0:
                        parentConst = \
                        cmds.parentConstraint(self.ikJointList[n], self.fkJointList[n], self.skinJointList[n],
                                              maintainOffset=True,
                                              name=side + self.userGuideName + "_" + self.jNameList[
                                                  n] + "_IkFkBlend_ParentConstraint")[0]
                        if n == 1:
                            revNode = cmds.createNode('reverse',
                                                      name=side + self.userGuideName + "_" + self.limbType.capitalize() + "_Rev")
                            cmds.connectAttr(
                                self.worldRef + "." + side + self.limbType + str(dpAR_count) + '_IkFkBlend',
                                revNode + ".inputX", force=True)
                        else:
                        ...
                        cmds.connectAttr(self.worldRef + "." + side + self.limbType + str(dpAR_count) + '_IkFkBlend',
                                         parentConst + "." + self.fkJointList[n] + "W1", force=True)
                        cmds.connectAttr(revNode + '.outputX', parentConst + "." + self.ikJointList[n] + "W0",
                                         force=True)
                # organize the ikFkBlend from before to limb:
                cmds.parentConstraint(self.fkCtrlList[0], self.ikJointList[0], maintainOffset=True,
                                      name=self.ikJointList[0] + "_ParentConstraint")
                cmds.parentConstraint(self.fkCtrlList[0], self.ikNSJointList[0], maintainOffset=True,
                                      name=self.ikJointList[0] + "_ParentConstraint")
                cmds.parentConstraint(self.fkCtrlList[0], self.fkJointList[0], maintainOffset=True,
                                      name=self.fkJointList[0] + "_ParentConstraint")
                cmds.parentConstraint(self.fkCtrlList[0], self.skinJointList[0], maintainOffset=True,
                                      name=self.skinJointList[0] + "_ParentConstraint")

                # creating ik controls:
                self.ikExtremCtrl = ctrls.cvBox(ctrlName=side + self.userGuideName + "_" + extremName + "_Ik_Ctrl",
                                                r=self.ctrlRadius * 0.5)
                if self.limbType == self.langDic[self.langName]['m028_arm']:
                    self.ikCornerCtrl = ctrls.cvElbow(
                        ctrlName=side + self.userGuideName + "_" + cornerName + "_Ik_Ctrl", r=self.ctrlRadius * 0.5)
                    cmds.setAttr(self.ikExtremCtrl + ".rotateOrder", 2)
                else:
                    self.ikCornerCtrl = ctrls.cvKnee(ctrlName=side + self.userGuideName + "_" + cornerName + "_Ik_Ctrl",
                                                     r=self.ctrlRadius * 0.5)
                    cmds.setAttr(self.ikExtremCtrl + ".rotateOrder", 3)
                cmds.addAttr(self.ikCornerCtrl, longName='active', attributeType='float', minValue=0, maxValue=1,
                             defaultValue=1, keyable=True);
                ...
                cmds.connectAttr(self.worldRef + "." + side + self.limbType + str(dpAR_count) + '_IkFkBlend',
                                 self.zeroFkCtrlList[1] + ".visibility", force=True)
                cmds.connectAttr(side + self.userGuideName + "_" + self.limbType.capitalize() + "_Rev" + ".outputX",
                                 self.ikCornerCtrlZero + ".visibility", force=True)
                cmds.connectAttr(side + self.userGuideName + "_" + self.limbType.capitalize() + "_Rev" + ".outputX",
                                 self.ikExtremCtrlZero + ".visibility", force=True)
                ...
                if self.limbStyle == self.langDic[self.langName]['m043_quadSpring']:
                    ...
                    if loaded:
                        ...
                        ikHandleMainList = cmds.ikHandle(
                            name=side + self.userGuideName + "_" + self.limbType.capitalize() + "_IkHandle",
                            startJoint=self.ikJointList[1], endEffector=self.ikJointList[len(self.ikJointList) - 2],
                            solver='ikSpringSolver')
                        ikHandleNotStretchList = cmds.ikHandle(
                            name=side + self.userGuideName + "_" + self.limbType.capitalize() + "_NotStretch_IkHandle",
                            startJoint=self.ikNSJointList[1],
                            endEffector=self.ikNSJointList[len(self.ikNSJointList) - 2], solver='ikSpringSolver')
                    else:
                        ...
                        ikHandleNotStretchList = cmds.ikHandle(
                            name=side + self.userGuideName + "_" + self.limbType.capitalize() + "_NotStretch_IkHandle",
                            startJoint=self.ikNSJointList[1],
                            endEffector=self.ikNSJointList[len(self.ikNSJointList) - 2], solver='ikRPsolver')
                else:
                    ...
                    ikHandleNotStretchList = cmds.ikHandle(
                        name=side + self.userGuideName + "_" + self.limbType.capitalize() + "_NotStretch_IkHandle",
                        startJoint=self.ikNSJointList[1], endEffector=self.ikNSJointList[len(self.ikNSJointList) - 2],
                        solver='ikRPsolver')
                ...
                ikHandleNotStretchGrp = cmds.group(empty=True,
                                                   name=side + self.userGuideName + "_NotStretch_IkHandle_Grp")
                ...
                self.ikHandlePointConst = \
                cmds.pointConstraint(self.ikExtremCtrl, ikHandleMainList[0], maintainOffset=True,
                                     name=ikHandleMainList[0] + "_ParentConstraint")[0]
                self.ikHandlePointConstList.append(self.ikHandlePointConst)
                cmds.orientConstraint(self.ikExtremCtrl, self.ikJointList[len(self.ikJointList) - 2],
                                      maintainOffset=True,
                                      name=self.ikJointList[len(self.ikJointList) - 2] + "_OrientConstraint")
                ctrls.setLockHide([self.ikExtremCtrl], ['sx', 'sy', 'sz'])
                cmds.pointConstraint(self.ikExtremCtrl, ikHandleNotStretchList[0], maintainOffset=True,
                                     name=ikHandleNotStretchList[0] + "_PointConstraint")[0]
                cmds.orientConstraint(self.ikExtremCtrl, self.ikNSJointList[len(self.ikNSJointList) - 2],
                                      maintainOffset=True,
                                      name=self.ikNSJointList[len(self.ikNSJointList) - 2] + "_OrientConstraint")
                ...
                cmds.delete(cmds.aimConstraint(self.ikJointList[1], poleVectorLoc, aimVector=(1.0, 0.0, 0.0),
                                               upVector=(0.0, 1.0, 0.0), worldUpType="object",
                                               worldUpObject=self.ikJointList[1], maintainOffset=False))

                # corner look at base:
                cmds.delete(cmds.aimConstraint(baseMiddlePointList[1], poleVectorLoc, aimVector=(0.0, 0.0, -1.0),
                                               upVector=(0.0, 1.0, 0.0), maintainOffset=False))
                ...
                poleVectorConstA = cmds.poleVectorConstraint(self.ikCornerCtrl, ikHandleMainList[0], weight=1.0,
                                                             name=ikHandleMainList[0] + "_PoleVectorConstraint")
                poleVectorConstB = cmds.poleVectorConstraint(self.ikCornerCtrl, ikHandleNotStretchList[0], weight=1.0,
                                                             name=ikHandleNotStretchList[0] + "_PoleVectorConstraint")
                cmds.connectAttr(self.ikCornerCtrl + '.active', poleVectorConstA[0] + "." + self.ikCornerCtrl + "W0",
                                 force=True)
                cmds.connectAttr(self.ikCornerCtrl + '.active', poleVectorConstB[0] + "." + self.ikCornerCtrl + "W0",
                                 force=True)

                # create annotation:
                annotLoc = \
                cmds.spaceLocator(name=side + self.userGuideName + "_" + self.limbType.capitalize() + "_Ant_Loc",
                                  position=(0, 0, 0))[0]
                annotation = cmds.annotate(annotLoc, tx="", point=cornerPos)
                annotation = cmds.listRelatives(annotation, parent=True)[0]
                annotation = cmds.rename(annotation,
                                         side + self.userGuideName + "_" + self.limbType.capitalize() + "_Ant")
                ...
                cmds.aimConstraint(annotLoc, self.ikCornerCtrl, aimVector=(0.0, 0.0, -1.0), upVector=(0.0, 1.0, 0.0),
                                   name=self.ikCornerCtrl + "_AimConstraint")
                ctrls.setLockHide([self.ikCornerCtrl], ['rx', 'ry', 'rz', 'sx', 'sy', 'sz', 'v'])
                self.cornerGrp = cmds.group(empty=True,
                                            name=side + self.userGuideName + "_" + self.limbType.capitalize() + "_PoleVector_Grp",
                                            absolute=True)
                self.cornerOrientGrp = cmds.group(empty=True,
                                                  name=side + self.userGuideName + "_" + self.limbType.capitalize() + "_PoleVectorOrient_Grp",
                                                  absolute=True)
                ...
                if self.limbStyle == self.langDic[self.langName]['m042_default']:
                    self.cornerOrient = \
                    cmds.orientConstraint(self.cornerOrientGrp, self.ikExtremCtrl, self.cornerGrp, skip=("y", "z"),
                                          maintainOffset=True, name=self.cornerGrp + "_OrientConstraint")[0]
                    ...
                    if self.limbType == self.langDic[self.langName]['m028_arm']:
                        ...
                        self.cornerOrient = \
                        cmds.orientConstraint(self.cornerOrientGrp, self.ikExtremCtrl, self.cornerGrp, skip=("x", "z"),
                                              maintainOffset=True, name=self.cornerGrp + "_OrientConstraint")[0]
                self.cornerOrientRev = cmds.createNode('reverse', name=side + self.userGuideName + "_CornerOrient_Rev")
                cmds.connectAttr(self.ikCornerCtrl + '.' + self.langDic[self.langName]['c_autoOrient'],
                                 self.cornerOrientRev + ".inputX", force=True)
                cmds.connectAttr(self.cornerOrientRev + '.outputX',
                                 self.cornerOrient + "." + self.cornerOrientGrp + "W0", force=True)
                cmds.connectAttr(self.ikCornerCtrl + '.' + self.langDic[self.langName]['c_autoOrient'],
                                 self.cornerOrient + "." + self.ikExtremCtrl + "W1", force=True)

                # working with follow of poleVector:
                self.cornerPoint = \
                cmds.pointConstraint(self.cornerOrientGrp, self.ikExtremCtrl, self.cornerGrp, maintainOffset=True,
                                     name=self.cornerGrp + "_ParentConstraint")[0]
                cmds.addAttr(self.ikCornerCtrl, longName=self.langDic[self.langName]['c_Follow'], attributeType='float',
                ...
                cmds.connectAttr(self.ikCornerCtrl + '.' + self.langDic[self.langName]['c_Follow'],
                                 self.cornerPointRev + ".inputX", force=True)
                cmds.connectAttr(self.cornerPointRev + '.outputX', self.cornerPoint + "." + self.cornerOrientGrp + "W0",
                                 force=True)
                cmds.connectAttr(self.ikCornerCtrl + '.' + self.langDic[self.langName]['c_Follow'],
                                 self.cornerPoint + "." + self.ikExtremCtrl + "W1", force=True)
                ...
                # creating attributes:
                cmds.addAttr(self.ikExtremCtrl, longName="stretchable", attributeType='float', minValue=0,
                             defaultValue=1, keyable=True)
                cmds.addAttr(self.ikExtremCtrl, longName="stretchType", attributeType='enum',
                             enumName="negative:positive:both", defaultValue=1, keyable=False)

                # creating distance betweens, multiplyDivides and reverse nodes:
                self.distBetweenList = ctrls.distanceBet(self.ikJointList[1], self.ikStretchExtremLoc,
                                                         name=side + self.userGuideName + "_" + kNameList[
                                                             1] + "_DistBet", keep=True)
                ...
                self.stretchIkFkMultDiv = cmds.createNode('multiplyDivide',
                                                          name=side + self.userGuideName + "_" + kNameList[
                                                              1] + "_StretchIkFk_MD")
                ...
                self.stretchOnOffMultDiv = cmds.createNode('multiplyDivide',
                                                           name=side + self.userGuideName + "_" + kNameList[
                                                               1] + "_StretchOnOff_MD")
                cmds.connectAttr(self.stretchIkFkMultDiv + '.outputX', self.stretchOnOffMultDiv + ".input1X",
                                 force=True)
                ...
                self.stretchRev = cmds.createNode('reverse',
                                                  name=side + self.userGuideName + "_" + kNameList[1] + "_Stretch_Rev")
                cmds.connectAttr(self.stretchOnOffMultDiv + '.outputX', self.stretchRev + ".inputX", force=True)
                cmds.connectAttr(self.stretchOnOffMultDiv + '.outputX',
                                 self.distBetweenList[5] + "." + self.ikStretchExtremLoc + "W0", force=True)
                cmds.connectAttr(self.stretchRev + '.outputX',
                                 self.distBetweenList[5] + "." + self.distBetweenList[4] + "W1", force=True)

                # here we calculate the stretch comparing with the current distance result:
                self.stretchMultDiv = cmds.createNode('multiplyDivide',
                                                      name=side + self.userGuideName + "_" + kNameList[
                                                          1] + "_Stretch_MD")
                cmds.connectAttr(self.distBetweenList[1] + '.distance', self.stretchMultDiv + ".input1X", force=True)

                startStretchValue = ctrls.distanceBet(self.ikJointList[1], self.ikJointList[2], keep=False)[0] + \
                                    ctrls.distanceBet(self.ikJointList[2], self.ikStretchExtremLoc, keep=False)[0]
                ...
                self.stretchCond = cmds.createNode('condition',
                                                   name=side + self.userGuideName + "_" + kNameList[1] + "_Stretch_Cnd")
                ...
                # both     (2) = 1
                # else     (x) = 2
                self.stretchCondOp0 = cmds.createNode('condition', name=side + self.userGuideName + "_" + kNameList[
                    1] + "_StretchOp0_Cnd")
                self.stretchCondOp1 = cmds.createNode('condition', name=side + self.userGuideName + "_" + kNameList[
                    1] + "_StretchOp1_Cnd")
                self.stretchCondOp2 = cmds.createNode('condition', name=side + self.userGuideName + "_" + kNameList[
                    1] + "_StretchOp2_Cnd")
                ...
                cmds.parentConstraint(self.skinJointList[0], self.distBetweenList[4], maintainOffset=True,
                                      name=self.distBetweenList[4] + "_ParentConstraint")
                ...
                if self.limbType == self.langDic[self.langName]['m028_arm'] and self.getHasBend() == False:
                    ...
                    forearmJnt = cmds.duplicate(self.skinJointList[2],
                                                name=side + self.userGuideName + "_" + self.langDic[self.langName][
                                                    'c_forearm'] + self.jSufixList[0])[0]
                    # delete its children:
                    ...
                    # create forearmCtrl:
                    forearmCtrl = cmds.circle(
                        name=side + self.userGuideName + "_" + self.langDic[self.langName]['c_forearm'] + "_Ctrl",
                        ch=False, o=True, nr=(0, 0, 1), d=1, s=8, radius=(self.ctrlRadius * 0.75))[0]
                    forearmGrp = cmds.group(forearmCtrl,
                                            name=side + self.userGuideName + "_" + self.langDic[self.langName][
                                                'c_forearm'] + "_Grp")
                    forearmZero = cmds.group(forearmGrp,
                                             name=side + self.userGuideName + "_" + self.langDic[self.langName][
                                                 'c_forearm'] + "_Zero")
                    tempToDelete = cmds.parentConstraint(forearmJnt, forearmZero, maintainOffset=False)
                    cmds.delete(tempToDelete)
                    cmds.parentConstraint(self.skinJointList[2], forearmZero, maintainOffset=True,
                                          name=forearmZero + "_ParentConstraint")
                    cmds.orientConstraint(forearmCtrl, forearmJnt, skip=["x", "y"], maintainOffset=True,
                                          name=forearmJnt + "_OrientConstraint")
                    # create attribute to forearm autoRotate:
                    cmds.addAttr(forearmCtrl, longName=self.langDic[self.langName]['c_autoOrient'],
                                 attributeType='float', minValue=0, maxValue=1, defaultValue=0.75, keyable=True)
                    ctrls.setLockHide([forearmCtrl], ['tx', 'ty', 'tz', 'rx', 'ry', 'sx', 'sy', 'sz', 'v'])
                    # make rotate connections:
                    forearmMD = cmds.createNode('multiplyDivide',
                                                name=side + self.userGuideName + "_" + self.langDic[self.langName][
                                                    'c_forearm'] + "_MD")
                    cmds.connectAttr(forearmCtrl + '.' + self.langDic[self.langName]['c_autoOrient'],
                                     forearmMD + '.input1X')
                ...
                self.ikFkBlendGrpToRevFoot = cmds.group(empty=True,
                                                        name=side + self.userGuideName + "_IkFkBlendGrpToRevFoot_Grp")
                self.ikFkBlendGrpToRevFootList.append(self.ikFkBlendGrpToRevFoot)
                tempToDelToRFpc = cmds.parentConstraint(self.ikExtremCtrl, self.ikFkBlendGrpToRevFoot,
                ...
                cmds.parentConstraint(self.ikExtremCtrl, self.fkCtrlList[len(self.fkCtrlList) - 1],
                                      self.ikNSJointList[-2], self.ikFkBlendGrpToRevFoot, maintainOffset=True,
                                      name=self.ikFkBlendGrpToRevFoot + "_ParentConstraint")[0]
                self.parentConstToRFOffsetList.append(parentConstToRFOffset)
                cmds.connectAttr(self.worldRef + "." + side + self.limbType + str(dpAR_count) + '_IkFkBlend',
                                 parentConstToRFOffset + "." + self.fkCtrlList[len(self.fkCtrlList) - 1] + "W1",
                                 force=True)
                ...
                if self.limbStyle != self.langDic[self.langName]['m042_default']:
                    ...
                    if self.limbStyle == self.langDic[self.langName]['m043_quadSpring']:
                        ...
                        self.fixIkSpringSolverGrp = cmds.group(self.ikJointList[1],
                                                               name=side + self.userGuideName + "_IkFixSpringSolver_Grp")
                        self.fixIkSpringSolverGrpList.append(self.fixIkSpringSolverGrp)
                        cmds.setAttr(self.fixIkSpringSolverGrp + ".visibility", 0)
                        cmds.parentConstraint(self.ikJointList[0], self.ikJointList[1], maintainOffset=True,
                                              name=self.ikJointList[1] + "_ParentConstraint")
                ...
                stretchDifMD = cmds.shadingNode('multiplyDivide', asUtility=True,
                                                name=side + self.userGuideName + "_Stretchable_Dif_MD")
                ...
                ikStretchCtrlCnd = cmds.shadingNode('condition', asUtility=True,
                                                    name=side + self.userGuideName + "_ikStretchCtrl_Cnd")
                ...
                cmds.connectAttr(ikStretchCtrlCnd + ".outColorR",
                                 parentConstToRFOffset + "." + self.ikExtremCtrl + "W0", force=True)

                ikStretchDifPMA = cmds.shadingNode('plusMinusAverage', asUtility=True,
                                                   name=self.userGuideName + "_Stretch_Dif_PMA")
                ...
                ikStretchCnd = cmds.shadingNode('condition', asUtility=True,
                                                name=side + self.userGuideName + "_ikStretch_Cnd")
                ...
                ikStretchClp = cmds.shadingNode('clamp', asUtility=True,
                                                name=side + self.userGuideName + "_IkStretch_Clp")
                cmds.setAttr(ikStretchClp + ".maxR", 1)
                cmds.connectAttr(ikStretchCnd + ".outColorR", ikStretchClp + ".inputR", force=True)
                cmds.connectAttr(ikStretchClp + ".outputR", parentConstToRFOffset + "." + self.ikNSJointList[-2] + "W2",
                                 force=True)
                ...
                if self.limbType == self.langDic[self.langName]['m028_arm']:
                    ...
                    if self.getHasBend() == True:
                        ...
                        self.toCtrlHookGrp = cmds.group(self.zeroFkCtrlGrp, self.zeroCornerGrp, self.ikExtremCtrlZero,
                                                        self.cornerOrientGrp, distBetGrp, self.origFromList[0],
                                                        self.origFromList[1], self.ikFkBlendGrpToRevFoot, self.worldRef,
                                                        name=side + self.userGuideName + "_Control_Grp")
                    else:
                        # use forearm control
                        self.toCtrlHookGrp = cmds.group(self.zeroFkCtrlGrp, self.zeroCornerGrp, self.ikExtremCtrlZero,
                                                        self.cornerOrientGrp, forearmZero, distBetGrp,
                                                        self.origFromList[0], self.origFromList[1],
                                                        self.ikFkBlendGrpToRevFoot, self.worldRef,
                                                        name=side + self.userGuideName + "_Control_Grp")
                else:
                    self.toCtrlHookGrp = cmds.group(self.zeroFkCtrlGrp, self.zeroCornerGrp, self.ikExtremCtrlZero,
                                                    self.cornerOrientGrp, distBetGrp, self.origFromList[0],
                                                    self.origFromList[1], self.ikFkBlendGrpToRevFoot, self.worldRef,
                                                    name=side + self.userGuideName + "_Control_Grp")
                self.toScalableHookGrp = cmds.group(self.skinJointList[0], self.ikJointList[0], self.fkJointList[0],
                                                    self.ikNSJointList[0],
                ...
                cmds.parentConstraint(self.toCtrlHookGrp, self.toScalableHookGrp, maintainOffset=True,
                                      name=self.toScalableHookGrp + "_ParentConstraint")
                self.toStaticHookGrp = cmds.group(self.toCtrlHookGrp, self.toScalableHookGrp, ikHandleGrp,
                                                  ikHandleNotStretchGrp, name=side + self.userGuideName + "_Grp")

                # new ribbon feature by James do Carmo, thanks!
                # not using bend or ikFkSnap systems to quadruped
                if self.limbStyle != self.langDic[self.langName]['m037_quadruped'] and self.limbStyle != \
                        self.langDic[self.langName]['m043_quadSpring']:
                    ...
                    if self.getHasBend():
                        ...
                        if name == self.langDic[self.langName]['c_leg_main']:  # leg
                            if s == 0:  # left side (or first side = original)
                                cmds.delete(cmds.aimConstraint(corner, loc, mo=False, weight=2, aimVector=(1, 0, 0),
                        ...
                        if self.limbType == self.langDic[self.langName]['m028_arm']:
                            if s == 0:
                                self.bendGrps = rb.addRibbonToLimb(prefix, name, loc, iniJoint, 'x', num, mirror=False,
                                                                   ctrlRadius=(self.ctrlRadius * 0.5), arm=True,
                                                                   worldRef=self.worldRef)
                            else:
                                self.bendGrps = rb.addRibbonToLimb(prefix, name, loc, iniJoint, 'x', num, mirror=False,
                                                                   ctrlRadius=(self.ctrlRadius * 0.5), side=1, arm=True,
                                                                   worldRef=self.worldRef)
                        else:
                            if s == 0:
                                self.bendGrps = rb.addRibbonToLimb(prefix, name, loc, iniJoint, 'x', num, mirror=False,
                                                                   ctrlRadius=(self.ctrlRadius * 0.5), arm=False,
                                                                   worldRef=self.worldRef)
                            else:
                                self.bendGrps = rb.addRibbonToLimb(prefix, name, loc, iniJoint, 'x', num, mirror=False,
                                                                   ctrlRadius=(self.ctrlRadius * 0.5), side=1,
                                                                   arm=False, worldRef=self.worldRef)
                        cmds.delete(loc)
                        ...
                        if self.bendGrpList:
                            if not cmds.objExists(self.worldRef + ".bends"):
                                cmds.addAttr(self.worldRef, longName='bends', attributeType='long', minValue=0,
                                             maxValue=1, defaultValue=1, keyable=True)
                                cmds.addAttr(self.worldRef, longName='extraBends', attributeType='long', minValue=0,
                                             maxValue=1, defaultValue=0, keyable=True)
                            ...
                            for extraBendGrp in self.extraBendList:
                                cmds.connectAttr(self.worldRef + ".extraBends", extraBendGrp + ".visibility",
                                                 force=True)
                ...
                if loadedIkFkSnap:
                    ...
                    ikFkNet = cmds.rename(ikFkNet.__melobject__(),
                                          side + self.userGuideName + "_" + str(dpAR_count) + "_IkFkNetwork")
                    self.ikFkNetworkList.append(ikFkNet)
                    if self.limbStyle != self.langDic[self.langName]['m037_quadruped'] and self.limbStyle != \
                            self.langDic[self.langName]['m043_quadSpring']:
                        lastIndex = len(otherCtrlList)
                        if self.getHasBend():
                            cmds.connectAttr(self.bendGrps['ctrlList'][0] + ".message",
                                             ikFkNet + ".otherCtrls[" + str(lastIndex + 1) + "]", force=True)
                            cmds.connectAttr(self.bendGrps['ctrlList'][1] + ".message",
                                             ikFkNet + ".otherCtrls[" + str(lastIndex + 2) + "]", force=True)
                            cmds.connectAttr(self.bendGrps['ctrlList'][2] + ".message",
                                             ikFkNet + ".otherCtrls[" + str(lastIndex + 3) + "]", force=True)
                        elif self.limbType == self.langDic[self.langName]['m028_arm']:
                            cmds.connectAttr(forearmCtrl + ".message",
                                             ikFkNet + ".otherCtrls[" + str(lastIndex + 1) + "]", force=True)
