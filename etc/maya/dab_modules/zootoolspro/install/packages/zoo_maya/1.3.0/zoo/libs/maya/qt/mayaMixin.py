#!/usr/bin/env python
"""
Copied from Maya 2020 Folder so we can have our own custom behaviour
...
class MayaQWidgetBaseMixin(object):
    ...
    def __init__(self, parent=None, *args, **kwargs):
        super(MayaQWidgetBaseMixin, self).__init__(parent=parent, *args,
                                                   **kwargs)  # Init all baseclasses (including QWidget) of the main class
...
class MayaQDockWidget(MayaQWidgetBaseMixin, QDockWidget):
    ...
    def __init__(self, parent=None, *args, **kwargs):
        super(MayaQDockWidget, self).__init__(parent=parent, *args,
                                              **kwargs)  # Init all baseclasses (including QWidget) of the main class
        ...
        try:
            ...
            mel.eval(
                'evalDeferred("updateEditorToggleCheckboxes()")')  # Currently mainWindow.handleDockWidgetVisChange only makes this updateEditorToggleCheckboxes call
    ...
    def setArea(self, area):
        ...
        mainWindow = self.parent() if isinstance(self.parent(), QMainWindow) else wrapInstance(
            long(omui.MQtUtil.mainWindow()), QMainWindow)
...
class MayaQWidgetDockableMixin(MayaQWidgetBaseMixin):
    ...
    def __init__(self, parent=None, *args, **kwargs):
        ...
        if parent is not None and long(getCppPointer(parent)[0]) == long(omui.MQtUtil.mainWindow()):
            parent = None
        super(MayaQWidgetDockableMixin, self).__init__(parent=parent, *args,
                                                       **kwargs)  # Init all baseclasses (including QWidget) of the main class

    def setDockableParameters(self, dockable=None, floating=None, area=None, allowedArea=None, width=None,
                              widthSizingProperty=None, minWidth=None, height=None, heightSizingProperty=None, x=None,
                              y=None, retain=True, plugins=None, controls=None, uiScript=None, closeCallback=None,
                              *args, **kwargs):
            ...
            if dockable == True and not self.isDockable():
                ...
                if floating == True or area is None:
                    ...
                    if self.parent() is None or (
                            long(getCppPointer(self.parent())[0]) == long(omui.MQtUtil.mainWindow())):
                        ...
                        if foundParentWorkspaceControl == False:
                            ...
                            if minWidth is None:
                                workspaceControlName = cmds.workspaceControl(workspaceControlName,
                                                                             ...
                                                                             loadImmediately=True, floating=True,
                                                                             initialWidth=width,
                                                                             widthProperty=widthSizingProperty,
                                                                             initialHeight=height,
                                                                             heightProperty=heightSizingProperty,
    ...
    def hide(self, *args, **kwargs):
        ...
        if parent:
            ...
            if parentName and len(parentName) and cmds.workspaceControl(parentName, q=True, exists=True):
                ...
                QWidget.setVisible(self,
                                   False)  # NOTE: Explicitly calling QWidget.setVisible() as using super() breaks in PySide: super(self.__class__, self).show()
    ...
    def raise_(self):
        ...
        if parent:
            ...
            if parentName and len(parentName) and cmds.workspaceControl(parentName, q=True, exists=True):
                ...
                QWidget.raise_(
                    self)  # NOTE: Explicitly using QWidget as using super() breaks in PySide: super(self.__class__, self).show()
    ...
    def setWindowTitle(self, val):
        ...
        '''
        # Handle the standard setVisible() operation of show()
        QWidget.setWindowTitle(self,
                               val)  # NOTE: Explicitly calling QWidget.setWindowTitle() as using super() breaks in PySide: super(self.__class__, self).show()
