...
class TrSysState(object):
    ...
    def resolveOS (self):
        if sys.platform == 'win32':
            self.osType = "Windows"
            p = self.getWinPlatformFromRegistry() # rather than platform.platform()
            ...
            self.osInfo = "Windows, " + w + ", " + self.osExtPlatform
    ...
    def getWinPlatformFromRegistry (self):
        ...
        # awkward about it, depending primarily on a 3rd party C module that we
        # don't ship.
        try:
            import _winreg
            vkey = _winreg.OpenKeyEx(_winreg.HKEY_LOCAL_MACHINE,
                              "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion")
            pname = _winreg.QueryValueEx(vkey, "ProductName")
            vnum = _winreg.QueryValueEx(vkey, "CurrentVersion")
            vbuild = _winreg.QueryValueEx(vkey, "CurrentBuild")
            _winreg.CloseKey(vkey)
            p = str(pname[0])+'-'+str(vnum[0])+'.'+str(vbuild[0])
            p = p.replace(' ', '-')
    ...
    class TrGPUInfo (object):
        ...
        def __init__(self):
            ...
            try:
                if sys.platform == 'win32':
                    self.resolveGPU_Windows()

                elif sys.platform == 'darwin':
                    self.resolveGPU_OSX()

                else:
                    self.resolveGPU_Linux()
        ...
        def resolveGPU_Linux (self):
            p = os.popen("/sbin/lspci -mm | fgrep 'VGA compatible'")
            vga = p.read().strip().split('\n')
            p.close()
            if vga:
                self.gpuCount = len(vga)
                id = vga[0].split('"')
                self.gpuLabel = id[5]+'; '+id[3]


        def resolveGPU_OSX (self):
            argv = ["/usr/sbin/system_profiler",
                  "SPDisplaysDataType",
                  "-detailLevel","mini"]
