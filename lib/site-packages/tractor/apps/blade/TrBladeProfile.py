...
#
import TractorSiteStatusFilter
...
class TrBladeProfile (object):
    ...
    def __init__(self, opts, appname, appvers, apprev, appdate):
        ...
        self.logger.info("GPU:  %s" % self.sysinfo.gpu.gpuLabel)
    ...
    def fetchProfiles (self, bstate, now):
        ...
        url = self.mkUrlParamsFromState( "config?q=profiles&file=" +
                                urllib2.quote(cfgfile), bstate, True )
    ...
    def fetchOtherConfigFile(self, filename, doparse=True, addopts=None):
        ...
        cfg = "config?q=get&file=" + urllib2.quote( filename )
    ...
    def applyProfile (self, pname, pd):
        ...
        for key in pd['Provides']:
            ...
            #   "pixar(max:3)" -- no more than 3 of these in use at a time
            #   "nuke(after:pixar)" -- nuke "filler" only after pixar main
            #
            # beware lists:  nuke(after:pixar,max:2)
            #
            p = key.find('(')
            if p >= 0:
                q = key.find(')', p)
    ...
    def setupLogging (self, logtemplate):
        ...
        if self.cmdOutputLogging != logtemplate:
            ...
            if logtype == "logserver":
                ...
                self.cmdOutputLogging = host + ':' + str(port)
