#
TrFileRevisionDate = "$DateTime: 2015/11/25 15:01:38 $"
...
def parseOptions (argv, progname, appVersionInfo, altUsage):
    ...
    optparser.add_option("--limit-tags", dest="tags",
            type="string", default="",
            help="specifies the tags to be added to each command; "
    ...
    optparser.add_option("--review", "--print-alfscript", "-o",
        ...
        if options.emitJSON:
            ...
            if options.loglevel == 0:
                ...
                if rc:
                    ...
                    print >>sys.stderr, "error parsing job script"
                else:
                    xcpt = "OK " + xcpt
...
def deliverToEngine (jfname, jobjson, jobalf, options):
    ...
    rr += "&jobAuthor=" + urllib.quote(options.jobAuthor)
    rr += "&format=JSON"
    if tsid:
        rr += "&tsid=" + tsid
...
def createArbTask (cmdargs, options, rangeValue=None, iterValue=None):
    ...
    title = options.ttitle or os.path.basename(cmdargs[0])
    argv = cmdargs[:]
    cmdstring = "{" + "} {".join(argv) + "}"
    tags = options.tags
    ...
    # ensure remote commands have a service key if no job service key
    if options.cmdmode == "RemoteCmd" and not svckey and not options.svckey: 
        svckey = "pixarRender"
