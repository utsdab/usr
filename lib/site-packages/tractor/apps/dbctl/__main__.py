...
import sys, os, subprocess, time, shutil, StringIO, ctypes, re, hashlib
...
class DBControlTool(CmdLineTool.BasicCmdLineTool):
        ...
        if self.isPostgreSQLRunning():
            ...
            if self.config.doDbStartup():
                ...
                if not self.isPostgreSQLRunning(maxAttempts=MAX_DB_CONNECT_ATTEMPTS):
                    raise DBControlToolError, "Failed to start a postgresql server on data directory %s.  A different server may be already running; to check, try 'ps -ax | grep postgres'.  Or another service is using port %d; to check, try 'sudo fuser %d/tcp', or 'sudo lsof -i:%d | grep LISTEN' on OSX.  %s may have more info." \
    ...
    def upgradeDB(self, upgrades):
        ...
        if ['ROLLBACK'] in result:
            raise DBControlToolError,\
                  "Upgrade failed.  Run 'tractor-dbctl --upgrade --debug' or inspect the postgres logs for details."
    ...
    def getUpgrades(self):
        ...
        except upgrade.UpgradeError, err:
    ...
    def execSQLWithResult(self, sql, user=SUPERUSER, dbname=None, *args):
        ...
        if rcode:
            self.dprint(err)
            raise ExecSQLError, "Failed to run %s with input: %s" % (str(argv), sql)
