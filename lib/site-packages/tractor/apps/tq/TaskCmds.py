...
class SkipTaskCmd(TaskOperationCmd):
    ...
    task's state to done and thus allows other tasks that were waiting on
    it to proceed.  Users are prompted before each task is skipped, unless
    the --yes flag is set.  Also, users can only skip tasks from their own
    jobs unless the --force flag is set, and the JobEditAccessPolicies
    in crews.config allows it.
    """
