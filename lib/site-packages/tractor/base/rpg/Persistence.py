...
#            inst = eval("%s.%s()" % (modname, classname))
    ...
    def __getValueType(self, val):
        ...
        if vtype in (types.ListType, types.TupleType, types.DictType,
        ...
        elif hasattr(types, "BooleanType") and vtype is types.BooleanType:
            return self.__typere.match(str(vtype)).group(1).lower()
        else:
    ...
    def __writeXMLNode(self, doc, node, value, level=0, indent='  ', root=0,
        ...
        if vtype in ('list', 'tuple'):
            ...
            for v in value:
                ...
                if readable and type(v) is types.StringType and \
            ...
            for k in keys:
                ...
                if readable and type(k) is types.StringType and \
    ...
    def readXMLString(self, xmlstr):
        ...
        try:
            dom = parseString(xmlstr)
                ...
                xml.parsers.expat.ExpatError), err:
...
class ClassCache(XMLObject):
    ...
    def addToCache(self, key, val, tstamp=True, save=True):
        ...
        if tstamp:
            self.__class__._cache[key] = (long(time.time()), val)
        else:
    ...
    def readCache(self, force=False):
        ...
        if self.__class__.cacheLife:
            now = time.time()
            for key,(tstamp,val) in self.__class__._cache.items():
