...
# compute these once for faster converting
_bytes_unitdefs = [1<<50L, 1<<40L, 1<<30L, 1<<20L, 1<<10L, 1]
...
# dict to associate unit with scaling
_parse_byte_factors = { \
        'p': 1<<50L, # one petabyte
        't': 1<<40L, # one terabyte
        'g': 1<<30L, # one gigabyte
        'm': 1<<20L, # one megabyte
        'k': 1<<10L, # one kilobyte
        'b': 1,      # one byte
