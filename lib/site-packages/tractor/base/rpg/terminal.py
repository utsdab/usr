...
class TerminalColor:
    ...
    def __init__(self, fg, bg=None):
        ...
        if not self.colorcodes['fg'].has_key(fg):
            raise UnknownColor, "'%s' is an unknown foreground color, must " \
                  "be one of %s" % (fg, self.colorcodes['fg'].keys())
        
        self.foreground = fg

        if bg and not self.colorcodes['bg'].has_key(bg):
            raise UnknownColor, "'%s' is an unknown background color, must " \
                  "be one of %s" % (bg, self.colorcodes['bg'].keys())
        self.background = bg
