...
class Formatter:
    ...
    def __init__(self, className, variables, separator=' ',
        ...
        self.abswidths = {}
        for key,val in self.widths.items():
            if val is not None:
            ...
            if len(vsplit) == 2:
                try:
                    ...
                    raise FormatterError, \
                          "expected integer in format variable: " + v
            elif len(vsplit) != 1:
    ...
    def _setFormatStrs(self):
        ...
        for v in self.variables:
            ...
            if varType is types.IntType or varType is types.LongType:
                varType = 'd'
            elif varType is types.FloatType:
                varType = 'f'
            else:
                varType = 's'
    ...
    def _parseFormatStr(self, formatStr):
        ...
        if len(fstrings.keys()) != len(self.variables):
    ...
    def printObject(self, obj, fitString='squeeze', fh=sys.stdout):
        ...
        print >> fh, self.getString(obj, fitString=fitString)
...
##                 width = whole
    ...
    def printDivider(self, dchar='=', fh=sys.stdout):
        ...
        print >> fh, self.getDivider(dchar=dchar)
    ...
    def printHeader(self, headers={}, clamp=0, fh=sys.stdout):
        ...
        print >> fh, self.getHeader(headers=headers, clamp=clamp)
