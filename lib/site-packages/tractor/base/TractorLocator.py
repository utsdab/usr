...
def main ():
    ...
    if action:
        ...
        print >>sys.stderr, "unknown TractorLocator stand-alone mode"
        sys.exit(2)
...
class TractorLocator (object):
    ...
    def Search (self):
        ...
        #
        if self.verbosity > 0:
            print >>sys.stderr, "probing:", self.serviceName
            ...
            while dt < maxwait:
                try:
                    ...
                    if r:
                        ...
                        if self.verbosity > 0:
                            print >>sys.stderr, "received", st, "from", sender
                            if self.verbosity > 1:
                    ...
                    if self.verbosity > 0:
                        print >>sys.stderr, "discovery wait error\n", e
        ...
        if not found:
            if self.verbosity > 0:
                print >>sys.stderr, "no service found:", self.serviceName
    ...
    def Notify (self, disposition):
        ...
        if self.verbosity > 0 and disposition == "alive":
            print >>sys.stderr, "mcast notify, advertising(" + \
                  self.serviceName + ")on %s:%d" % self.serviceAddr
    ...
    def OpenDashboard (self):
        ...
        try:
            ...
            ui = "https://renderman.pixar.com/forum/docs/Tractor_2/tractor_initial_config.php"
    ...
    def msearchResponse (self, sender, sdict, reply):
        ...
        if self.verbosity > 0:
            print >>sys.stderr, "answering M-SEARCH(%s) for" % sdict['ST'], sender
    ...
    def chkBootstrap (self, needNotify):
        ...
        if self.addrInitState > 2:
            if self.verbosity > 0:
                print >>sys.stderr, "mcast bootstrap failed, exit."
            return -1  # quit
        ...
        while keepRunning:
            ...
            for sock in readable:
                ...
                if self.addrInitState > 0 and "_TrTestIP_" in request:
                    ...
                    if self.verbosity > 1:
                        print >>sys.stderr, "IP addr bootstrap:", self.serviceAddr
    ...
    def resolveLocalHostAddr (self, hostport):
        ...
        if hp:
            try:
                ...
                if self.verbosity > 0:
                    print >>sys.stderr, "invalid port number:", hp, "(using 80)"
        ...
        if self.verbosity > 0:
            if e:
                print >>sys.stderr, "lookup error '"+hnm+"'", e, \
                                    "\ntrying fallback IP discovery"
            else:
                print >>sys.stderr, "resolved addr as %s:%d" % self.serviceAddr
