...
extension = "exr"
finalOutputImages = usedirmap("{}/images/{}/{}.%04d.{}".format(proj, scene, scene, extension))
...
job = author.Job(title="Maya Mental Ray with dirmap",
                 priority=100,
                 envkey=["maya2015"],
...
for chunk in range(start, end, by):
    ...
    rarg.append("-camera")  # render which camera to render
...
proxy = author.Task(title="Make Proxy - ffmpeg", argv=["ffmpeg",
                                                       ...
                                                       "-vcodec", "acpn",
                                                       "%s" % proxyOutput], service="MayaMentalRay")
