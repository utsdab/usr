...
job = author.Job(title="Maya_MR_v004",
                 priority=100,
                 envkey=["maya2015"],
         metadata=localuser,
...
"""
    ...
    libav vs ffmpeg guff....
    be careful of %04d not being understood correctly by ffmpeg 2.5.3
    i trust 2.2.4 so far
"""
proxy = author.Task(title="Make Proxy - ffmpeg", argv=["ffmpeg", "-f", "image2", "-r", "1",
                                                       "-i", "%s" % finalOutputImages,
                                                       "-codec:v", "prores", "-profile:v", "0",
                                                       "-s", "960x540","-threads","2","-r","25","-y",
